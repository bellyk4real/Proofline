...
class _QueryModel(_FakeModel):
    """Embedding fixture for a single query string."""

    def encode(self, texts: str, **kwargs: Any) -> np.ndarray:
        self.encode_kwargs = kwargs
...
def test_generates_normalized_query_embedding() -> None:
    model = _QueryModel()

    vector = generate_query_embedding("What is an AI system?", model)
...
def test_rejects_empty_question() -> None:
    with pytest.raises(ValueError, match="question"):
        generate_query_embedding("  ", _QueryModel())


def test_rejects_wrong_query_embedding_dimension() -> None:
    with pytest.raises(ValueError, match="384-dimensional"):
        generate_query_embedding("A question", _QueryModel(dimension=3))
